...
def capture_round(entry, frames, number, timeout):
    channel, url = entry
...
def card(item):
    ...
    return f'<section class="card"><h2>{html.escape(item.channel)}</h2><code>{html.escape(item.url)}</code><p><small>UTC：{timestamps}</small></p>{prior}{error}<div class="frames">{images}</div></section>'


def page(title, intro, items):
    ui = Path(__file__).with_name("review_ui.html").read_text(encoding="utf-8")
    return f'''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>{html.escape(title)}</title><style>body{{font:15px/1.5 system-ui,sans-serif;margin:24px;color:#222}}code{{word-break:break-all}}.card{{border:1px solid #ccc;border-radius:8px;padding:12px;margin:16px 0}}h2{{margin:0}}.frames{{display:flex;gap:8px;overflow:auto}}img{{width:320px;height:180px;object-fit:contain;background:#111}}.manual{{color:#075b24}}.error{{color:#a11}}.missing{{color:#777}}</style><body><h1>{html.escape(title)}</h1><p>{html.escape(intro)}</p>{''.join(card(item) for item in items)}{ui}</body></html>'''
...
def main():
    ...
    frames = args.output_dir / "frames"; frames.mkdir(parents=True); entries = read_entries(args.playlist); overrides = read_overrides(args.overrides)
    ...
    for number, delay in enumerate(delays, 1):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
            futures = {pool.submit(capture_round, entry, frames, number, args.timeout): entry for entry in entries}
            for future in concurrent.futures.as_completed(futures):
                entry = futures[future]; frame, at, error = future.result(); item = items[entry]; item.captured_at.append(at)
                if frame: item.frames.append(frame)
    ...
    intro = "每轮均重新连接，记录实际 UTC 采样时间。已有规则只是历史信息，新画面仍可重新审核。"
